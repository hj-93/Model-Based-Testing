...
public class Adapter {
    ...
    public Agent.MoveCoord proposeValidPlayerMove() {
        ...
        ArrayList<Agent.MoveCoord> suggestedMoves = reversiGame.findValidMove(reversiGame.getBoard(), playerPiece, true);
    ...
    public Agent.MoveCoord proposeInvalidPlayerMove() {
        ...
        for (int i = 0; i < reversiGame.sBOARD_SIZE; ++i) {
            for (int j = 0; j < reversiGame.sBOARD_SIZE; ++j) {
                if (reversiGame.getBoard()[i][j] != (playerPiece == reversiGame.sWHITE_PIECE ?
    ...
    public void tryEffectMove(Agent.MoveCoord coord) {
